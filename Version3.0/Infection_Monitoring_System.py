        ...
        while a:
            if str(self.table.cell(i, 1).value) == int_src:
                ...
                if int(self.table.cell(self.out, 0).value) == 0:
                    ...
                    self.result_Text.insert(1.0, "疾病名：%s传染性：无" % str(self.table.cell(self.out, 1).value))
                elif int(self.table.cell(self.out, 0).value) == 1:
                    self.result_Text.delete(1.0, END)
                    self.result_Text.insert(1.0, "姓名：%s传染性：甲型" % str(self.table.cell(self.out, 1).value))
                elif int(self.table.cell(self.out, 0).value) == 2:
                    self.result_Text.delete(1.0, END)
                    self.result_Text.insert(1.0, "姓名：%s传染性：乙型" % str(self.table.cell(self.out, 1).value))
                elif int(self.table.cell(self.out, 0).value) == 3:
                    self.result_Text.delete(1.0, END)
                    self.result_Text.insert(1.0, "姓名：%s传染性：丙型" % str(self.table.cell(self.out, 1).value))
                a = False
                break
...
class gui_new_disease():
    ...
    def set_new_disease_window(self):
        ...
        self.disease_name_Text.grid(row=1, column=15, rowspan=10, columnspan=10)
        ...
        #按钮
        self.str_trans_to_md5_button = Button(self.window, text="保存", bg="lightblue", width=10, command=self.save_button)
    ...
    def get(self):
        ...
        self.disease_id = int(self.disease_id_Text.get(1.0, END).strip().replace("\n", "").encode())

        self.disease_name = str(self.disease_name_Text.get(1.0, END).strip().replace("\n", "").encode())
...
class gui_root():
    ...
    def set_ims_window(self):
        ...
        self.diseasemenu.add_command(label="Search", command=self.search_window)
